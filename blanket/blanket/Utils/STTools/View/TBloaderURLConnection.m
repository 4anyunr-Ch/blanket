...
@property(nonatomic,strong)NSMutableArray            *beginArray;
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        self.beginArray = [NSMutableArray new];
        self.finshArray = [NSMutableArray new];
    }
    return self;
}
...
- (void)processPendingRequests
{
    ...
    NSArray * originArray = [NSArray arrayWithArray:self.pendingRequests];
    for (AVAssetResourceLoadingRequest *loadingRequest in originArray)
    {
...
- (BOOL)respondWithDataForRequest:(AVAssetResourceLoadingDataRequest *)dataRequest
{
    ...
    NSRange dataRange = NSMakeRange((NSUInteger)startOffset- self.task.offset, (NSUInteger)numberOfBytesToRespondWith);
    if (dataRange.length - dataRange.location > filedata.length ) {
        return NO;
    }
...
- (BOOL)resourceLoader:(AVAssetResourceLoader *)resourceLoader shouldWaitForLoadingOfRequestedResource:(AVAssetResourceLoadingRequest *)loadingRequest
{
    ...
    [self.beginArray addObject:loadingRequest.dataRequest];
...
{
  //  NSRange srange = NSMakeRange(100, 300);
    //NSLog(@"%lu---%lu",srange.location,srange.length);
...
{
    NSLog(@"完成了一个分段请求\n");
    [self.finshArray addObject:loadingRequest.dataRequest];
